...
type View =
  ...
  | { name: "form" }
  | { name: "opinions"; proposal: Proposal };
  ...
  if (view.name === "detail") {
    return (
      <ProposalDetail
        ...
        onOpenOpinions={(proposal) => setView({ name: "opinions", proposal })}
      />
    );
  }

  if (view.name === "opinions") {
    return (
      <ConsensusDevPage
        proposalId={view.proposal.id}
        proposalTitle={view.proposal.title}
        proposalDescription={view.proposal.content}
        currentUser={{
          id: user.id,
          name: user.name,
          company: "Meridian",
        ...
        onBack={() => setView({ name: "detail", id: view.proposal.id })}
      />
    );
  }
